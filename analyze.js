  ...
  const messages = [
    {
      ...
      content: dedent`
        ...
        Provide scores from 0 to 100 for each trait, where 0 is very low and 100 is very high. Also, provide a brief explanation for your assessment.
        The score should be in the format of a JSON: {o:1,c:2,e:3,a:4,n:5}, then add a separator "====" after the json.
        The explanantion should be in another json in the key value format.
      `.trim(),
    },
    {
      role: 'user',
...
export const parseAnalysisResponse = (responseContent) => {
  try {
    ...
    const explanation = JSON.parse(explanationPart);
